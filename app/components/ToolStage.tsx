...
export function LiveFrame({
  ...
  return (
    <div className="overflow-hidden rounded-[16px] border border-hair bg-surface">
      ...
      <div className={`relative ${heightClass} bg-bg`}>
        {!failed ? (
          ...
          <div className="absolute inset-0 flex flex-col items-start justify-end bg-[radial-gradient(ellipse_at_top,_rgba(20,184,166,0.12),_transparent_55%)] px-6 py-8 sm:px-10 sm:py-10">
            <img
              src="/yok-mark.png"
              alt=""
              width={72}
            ...
            {note ? (
              ...
              <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted">
                This tool is live. The host does not allow embedding here — open
        ...
        {tools.map((tool) => {
          ...
          return (
            <button
              ...
              className={
                on
                  ? "shrink-0 rounded-xl border border-accent/60 bg-accent/10 px-3 py-2 text-[11px] font-semibold tracking-wide text-accent"
