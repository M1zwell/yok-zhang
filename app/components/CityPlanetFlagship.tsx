...
export function CityPlanetFlagship({ locale = "en" }: { locale?: Locale }) {
  ...
  return (
    <article className="flagship-card relative overflow-hidden rounded-[24px] border border-accent/30 bg-deep p-5 shadow-[0_0_64px_rgba(20,184,166,0.16),0_0_80px_rgba(255,71,120,0.08)] sm:p-10">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div className="worlds-haze" />
      </div>
      <div className="relative grid items-start gap-6 md:grid-cols-[minmax(0,1fr)_auto]">
        <div className="min-w-0">
          ...
          <ul className="mt-8 flex flex-wrap gap-2">
            {[
              ...
              <li
                key={stat}
                className="inline-flex min-h-10 items-center rounded-full border border-accent/35 bg-accent/10 px-3 font-mono text-[11px] tracking-wide text-accent"
              >
                {stat}
              </li>
            ))}
        ...
        <div className="pointer-events-none relative mx-auto hidden shrink-0 md:block" aria-hidden>
