# Claim + social routine (planet reels)

**拍片请用 [`FILM_ON_PC.md`](./FILM_ON_PC.md) + [`SHOT_LIST.md`](./SHOT_LIST.md)。** 一条超长再切：[`LONG_TAKE.md`](./LONG_TAKE.md)。云端 Linux 成片太慢，不当 Hero。下面是发片时能说 / 不能说。

## Can we say “the 1st playable city planet with LIVE data”?

**Lead with the category, not “1st.”** Academic HK 3D sandboxes (e.g. UrbanGround) and other globe toys exist. Absolute firsts get challenged.

**Safer hero lines (pick one):**

- EN: “A playable city-planet with live map + weather data — walk Central for free.”
- EN: “Playable HD city-planets from open map data — walk it, flip the weather, pin a live spot.”
- CN: 「可玩的城市行星 · 活數據地圖」
- CN: 「免費走進中環 Cube-Belt · 天氣、車位、附近都是活數據」

### What LIVE must mean on-screen (we have the clip)

Use `06-highlights/03_live_data_pin_weather_parking.mp4` plus the pin still.

**Can say live / near-live:** OSM footprints on the planet; weather chips (Day/Night/Rain/Typhoon/Undersea); pin card with coords, **27°C / AQI / UV**, car parks, nearby POIs, “live data: HK gov open data”.

**Cannot say live:** landmarks still in PR, SSO, pay-to-win, GPS “Ask to be located” as live location.

**Never:** fake the locate confirm. We skipped it.

So: **yes** to “playable city planet + live data”; **no** to unbacked “world’s first” unless you later scope it tightly (still soft).

---

## Best social strategy (clip factory, not one mega-post)

Same beat **CN (小红书)** + **EN (X)**. Skip Reddit until captcha is easy.

| Lane | Cadence | Use these files |
| --- | --- | --- |
| Hero / acquisition | 3–4×/week X + XHS | `01_overview_central_diorama` · `10_paris_ile_de_la_cite` · `11_full_hud_overview` · still `still_coin_rally_nyc_street.png` |
| Feature HUD | 2×/week | `08_bastion_depths_inside` · `14_courier_rush_roster` · Kart HUD · Siege Wave 1 — **one verb per short** |
| Money honesty | 1×/week | Print $6 still + “Sign in to order” · never pay-to-win |
| BiP / trust | weekday desk | Depths delver · Bastion 0 walls · Cursor/debug, no spend |

**Routine shape**

- Weekday 09:00: one vertical from `06-highlights` 01 or 02 → XHS + X
- Midweek: how-to (guest walk / HUD keys I P M O / Print $6)
- Weekend: highlight (night + Siege or Sea if you recut a clean sea take)

Caption template: **verb + place + proof of live**. Example: “Walk Central. Flip to night. Pin the IFC — car parks and AQI are live.” / 「走進中環。切夜景。釘 IFC —— 車位和 AQI 是活的。」

---

## Flows worth a second pass (not fully in this library)

**Pass 3 smoothness:** recut keepers with `scripts/smooth-cut.sh` (crop chrome, 30fps, 1.45×). New 9s Remotion loops come from garden localhost `/film` (EN) and `/zh-Hans/film` (XHS). Live planet shorts (`localhost_overview_central`, `localhost_paris_overview`) were kiosk-filmed through `http://127.0.0.1:4173` (Chrome-UA cache). Do not post the 25fps 40s+ masters as the hero.

| Flow | Today | Film next |
| --- | --- | --- |
| Guest → walk | **Strongest truth** — Central + Paris + Tokyo catalog | Crop browser chrome for cinematic; keep one take *with* the tab for “runs in a browser” |
| Register | `/signup` 404; Jubit `/signin` works; Google 2FA filmed (`05-flows/signin_google_phone_verify.mp4`) | Complete phone verify on a trusted device, then film `☁️ Synced` |
| Refill ju | Pack picker exists; guest cannot open checkout | After Synced: click Refill ju, show $5/$10/$25, **do not pay** |
| Subscribe | Not on planet; ChatLab is a debate lab | If you show $8.88, label **ChatLab**, not planet P2W |
| HUD keywords | I / P / M / O / G visible | 8–12s cuts, one key each |
| Re-skin / Build 30 ju | Interior + Shop / 3D / Print filmed; empty-lot sheet still flaky | Open sheet, **no file, no ju spend** |
| Alt+Shift rotate | Attempted; not a clean isolated take | Dedicated 8s: hold Alt+Shift, drag canvas |
| Coin Rally | **Street still works** (`still_coin_rally_nyc_street.png` — Gas/Brake, 26 km/h). The 65s master later toasts **no connected drivable route**. `07` is NYC planet + route-building, not a clean drive. | Re-record 12s after the route exists; do not post Central’s “no road” take |
| Bastion Depths | **Entered** as guest — portal + hex dungeon + Jubit 3D (`08` / `09`) | Longer combat / Boss Lair if you want a boss reel |

---

## Caption bank (safe)

EN

1. Walk Central for free. The weather HUD is live.
2. Overview → Immersive. Same planet, two cameras.
3. Pin IFC. Car parks and AQI from HK open data.
4. Night on a city-planet. Not a screenshot — a tab.
5. Print the globe you walked. Desk 80 mm is $6. Not pay-to-win.
6. Drop me somewhere → Lower Manhattan. Same toy, another city.
7. Coin Rally on NYC: we have a street still (Gas/Brake). The 65s take later says no route — don’t post that toast.
8. Bastion Depths is a dungeon on the planet. Guest can walk in.

CN

1. 免費走中環。天氣條是活的。
2. 總覽切沉浸。同一顆行星，兩台相機。
3. 釘 IFC。車位、AQI 來自香港開放數據。
4. 城市行星的夜。不是渲染圖，是一個分頁。
5. 把你走過的行星打出來。桌上 80mm 美金 6。不是課金贏。
6. 「隨便丟我去」——下城曼哈頓。同一套玩具，另一座城。
7. 紐約 Coin Rally：有一張街頭油門剎車靜幀。65 秒母帶後面會跳出「還沒路」——別當完成片發。
8. 堡壘深淵是行星上的地牢。遊客也能走進去。
