...
import { isPlanningOwner } from "./gate";
...
export async function verifyPlanningOwner(session: FamilySession): Promise<boolean> {
  const jwt = decodeJwtPayload(session.accessToken);
  const claimed = session.email || jwt?.email || "";
  if (!isPlanningOwner(claimed)) return false;
  ...
  try {
    ...
    return isPlanningOwner(payload.email || payload.user?.email);
  } catch {
    return false;
  }
}
