...
import { isFundingOwner } from "./gate";
...
export async function verifyFundingOwner(session: FamilySession): Promise<boolean> {
  const jwt = decodeJwtPayload(session.accessToken);
  const claimed = session.email || jwt?.email || "";
  if (!isFundingOwner(claimed)) return false;
  ...
  try {
    ...
    return isFundingOwner(payload.email || payload.user?.email);
  } catch {
    return false;
  }
}
